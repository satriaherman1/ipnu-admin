...
interface IPostListProps {
  ...
  posts: IArticleData[];
  setOpenConfirmDelete: (confirmDelete: boolean) => void;
  header: string;
}
...
export default function PostListComponent(props: IPostListProps) {
  const { posts, setDeleteId, setOpenConfirmDelete, header } = props;
  const [order, setOrder] = React.useState<Order>("asc");
  const [orderBy, setOrderBy] = React.useState<keyof IArticleData>("title");
  ...
  const handleSelectAllClick = (event: React.ChangeEvent<HTMLInputElement>) => {
    if (event.target.checked) {
      const newSelecteds: any[] = posts?.map((n: any) => n.name);
  ...
  return (
    <Box sx={{ width: "100%" }}>
      <Paper sx={{ width: "100%", mb: 2 }}>
        <EnhancedTableToolbar header={header} numSelected={selected.length} />
        <TableContainer>
                          ...
                          <div className="flex">
                            ...
                            <Box sx={{ bgcolor: "error.main" }} className="rounded-lg ml-[20px]">
                              <Button
                                ...
                                color="error"
                                className="max-w-[10px] rounded-lg"
                              >
                                <Delete />
                              </Button>
                            </Box>
